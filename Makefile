...
$(SRC_DIR)/grammar.json: grammar.js
	$(TS) generate --no-parser --js-runtime=native $^

$(PARSER): $(SRC_DIR)/grammar.json
	$(TS) generate --abi=15 --js-runtime=native $^

install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/luau '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/luau
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/luau
